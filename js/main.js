(function () {
  ...
  const defaultMenu = [
    ...
    { name: "Plain Cheese Pizza", category: "pizza", description: "Classic melted cheese single size", isVeg: true, available: true, prices: {'regular': 90}, imageUrl: "https://images.unsplash.com/photo-1571066811602-71683a3f680d?q=80&w=600&auto=format&fit=crop" },
    ...
    { name: "Veg Spring Roll", category: "chinase", description: "Crispy rolls stuffed with mixed vegetables", isVeg: true, available: true, prices: {'single': 140}, imageUrl: "https://images.unsplash.com/photo-1544025162-d76694265947?q=80&w=600&auto=format&fit=crop" },
    { name: "Paneer Spring Roll", category: "chinase", description: "Crispy rolls stuffed with paneer and veggies", isVeg: true, available: true, prices: {'single': 180}, imageUrl: "https://images.unsplash.com/photo-1534422298391-e4f8c172dddb?q=80&w=600&auto=format&fit=crop" },
    { name: "Corn Crispy", category: "chinase", description: "Crispy fried sweet corn with peppers", isVeg: true, available: true, prices: {'single': 100}, imageUrl: "https://images.unsplash.com/photo-1541532713592-79a0317b6b77?q=80&w=600&auto=format&fit=crop" },
    { name: "Veg Crispy", category: "chinase", description: "Crispy batter fried assorted vegetables", isVeg: true, available: true, prices: {'single': 100}, imageUrl: "https://images.unsplash.com/photo-1626132647523-66f5bf380027?q=80&w=600&auto=format&fit=crop" },
    { name: "Paneer Crispy", category: "chinase", description: "Crispy fried batter-coated paneer strips", isVeg: true, available: true, prices: {'single': 240}, imageUrl: "https://images.unsplash.com/photo-1567188040759-fb8a883dc6d8?q=80&w=600&auto=format&fit=crop" },
    { name: "Mushroom Crispy", category: "chinase", description: "Batter-fried crispy mushrooms", isVeg: true, available: true, prices: {'single': 240}, imageUrl: "https://images.unsplash.com/photo-1568254183919-78a4e43a2877?q=80&w=600&auto=format&fit=crop" },
    { name: "Veg Hara Kabab", category: "chinase", description: "Crispy pan-fried spinach and green peas patties", isVeg: true, available: true, prices: {'single': 100}, imageUrl: "https://images.unsplash.com/photo-1601050690597-df056fb4ce78?q=80&w=600&auto=format&fit=crop" },
    { name: "Veg Cutlet", category: "chinase", description: "Golden crispy deep fried mixed vegetable patties", isVeg: true, available: true, prices: {'single': 100}, imageUrl: "https://images.unsplash.com/photo-1541832676-9b763b0239ab?q=80&w=600&auto=format&fit=crop" },
    { name: "Paneer Cutlet", category: "chinase", description: "Crispy deep fried spiced paneer and potato patties", isVeg: true, available: true, prices: {'single': 180}, imageUrl: "https://images.unsplash.com/photo-1565557623262-b51c2513a641?q=80&w=600&auto=format&fit=crop" },
    { name: "White Sauce Pasta", category: "chinase", description: "Pasta in creamy herbed white sauce", isVeg: true, available: true, prices: {'single': 140}, imageUrl: "https://images.unsplash.com/photo-1645112411341-6c4fd023714a?q=80&w=600&auto=format&fit=crop" },
    { name: "Red Sauce Pasta", category: "chinase", description: "Pasta in tangy tomato basil sauce", isVeg: true, available: true, prices: {'single': 120}, imageUrl: "https://images.unsplash.com/photo-1551183053-bf91a1d81141?q=80&w=600&auto=format&fit=crop" },
    { name: "Manchow Soup", category: "chinase", description: "Spicy and sour soup served with fried noodles", isVeg: true, available: true, prices: {'single': 80}, imageUrl: "https://images.unsplash.com/photo-1547592165-e1d17f8e05cc?q=80&w=600&auto=format&fit=crop" },
    { name: "Tomato Soup", category: "chinase", description: "Creamy tomato soup with herbed croutons", isVeg: true, available: true, prices: {'single': 80}, imageUrl: "https://images.unsplash.com/photo-1476718406336-bb5a9690ee2a?q=80&w=600&auto=format&fit=crop" },
    { name: "Lemon Coriander Soup", category: "chinase", description: "Light refreshing soup with lemon and coriander", isVeg: true, available: true, prices: {'single': 80}, imageUrl: "https://images.unsplash.com/photo-1603105037880-880cd4edfb0d?q=80&w=600&auto=format&fit=crop" },
  ...
  function seedDatabase() {
    if (typeof db !== "undefined") {
      db.ref("cityhut/cms/menu").once("value", menuSnap => {
        ...
        const isNewMenuSeeded = menuItemsList.length > 0 && menuItemsList.some(item => item.name === "Achari Do Pyaza Pizza");

        if (!isNewMenuSeeded) {
          console.log("New menu is not seeded. Seeding defaults...");
  ...
  function initCheckoutForm() {
    ...
    orderForm.addEventListener("submit", async (e) => {
      ...
      // Phone formatting and validation helper
      ...
      if (selectedMode === "delivery") {
        ...
        const rawPhone = document.getElementById("client-phone-delivery").value.trim();
        ...
        const formattedPhone = validateAndFormatPhone(rawPhone);
        if (!formattedPhone) {
          document.getElementById("client-phone-delivery").parentNode.querySelector(".form-error").style.display = "block";
          isValid = false;
        } else {
          phone = formattedPhone;
          document.getElementById("client-phone-delivery").value = "+" + formattedPhone;
        }
        ...
        const rawPhone = document.getElementById("client-phone-dt").value.trim();
    ...
    function calculateTotal() {
      ...
      overlay.querySelectorAll('.cust-crust-card').forEach(card => {
        ...
        if (input.value === "Cheese Burst") {
          const isValuePizza = product.prices && product.prices.small === 150;
          if (isValuePizza) {
            if (selectedSizeLabel === "Medium") price = 190;
            else if (selectedSizeLabel === "Large") price = 270;
            else price = 140;
          } else {
      ...
      if (activeCrustInput) {
        if (activeCrustInput.value === "Cheese Burst") {
          const isValuePizza = product.prices && product.prices.small === 150;
          if (isValuePizza) {
            if (selectedSizeLabel === "Medium") crustPrice = 190;
            else if (selectedSizeLabel === "Large") crustPrice = 270;
            else crustPrice = 140;
          } else {
      ...
      addBtn.addEventListener("click", () => {
        ...
        if (activeCrustInput) {
          ...
          if (crust === "Cheese Burst") {
            const isValuePizza = activeCustomizerProduct.prices && activeCustomizerProduct.prices.small === 150;
            if (isValuePizza) {
              if (size === "Medium") crustPrice = 190;
              else if (size === "Large") crustPrice = 270;
              else crustPrice = 140;
            } else {
